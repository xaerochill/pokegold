...
GymStatue1Script:
	readvar VAR_OBEDIENCE
	ifequal MAX_LEVEL, .ObedienceOff
; Obedience is ON — show level and offer to toggle off
	opentext
	writetext GymStatue_ObedienceOnText
	yesorno
	iffalse .Done
	special SetObedienceOff
	writetext GymStatue_ToggledOffText
	waitbutton
	closetext
	end
.ObedienceOff:
; Obedience is OFF — offer to toggle on
	opentext
	writetext GymStatue_ObedienceOffText
	yesorno
	iffalse .Done
	special RecalcObedienceLevel
	writetext GymStatue_ToggledOnText
	waitbutton
	closetext
	end
.Done:
